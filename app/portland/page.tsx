...
export function generateMetadata(): Metadata {
  const listings = getListingsByCity('Portland');
  const count = listings.length;
  const ratesWithValues = listings.map(l => l.assumableRate).filter(r => r > 0);
  const minRate = ratesWithValues.length > 0 ? Math.min(...ratesWithValues).toFixed(2) : '2.70';
  return {
    title: `Portland Assumable Mortgages | ${count} Homes | ${minRate}% Rate | The Assumable Guy`,
    description: `Browse ${count} assumable mortgage listings in Portland, OR. Rates as low as ${minRate}%. Save hundreds per month vs new loans at 6.9%. FHA and VA loans available.`,
    alternates: { canonical: `https://assumable-oregon-prototype.vercel.app/portland` },
  };
}

export default function PortlandPage() {
  const listings = getListingsByCity('Portland');
  // Pass minPrice: 0 to bypass the $350K default — Oregon prototype has listings below that
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="max-w-2xl mb-10">
          <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 mb-4">
            Assumable Mortgages in Portland, Oregon
          </h1>
          <p className="text-xl text-slate-600 mb-3">
            Browse {listings.length} assumable mortgage listings in Portland. Average assumable rate: {avgRate}%.
          </p>
        ...
        {listings.length > 6 && (
          <div className="text-center mt-8">
            <Link href={`/homes?city=${encodeURIComponent('Portland')}`} className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition">
              View All {listings.length} Portland Listings
            </Link>
          </div>
        )}
      </section>
